...
export type GenerationOutputMetadata = {
   ...
   * Useful for HTTP streaming deduplication: pass this to `skipStreamIds`
   * on `useUIMessages` / `useThreadMessages` to avoid showing the same
   * content twice (once from the HTTP stream and once from the persisted
   * delta stream).
   */
  streamId?: string;
  /**
