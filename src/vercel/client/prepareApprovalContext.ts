...
type Response = Extract<ToolPart, { type: "tool-approval-response" }>;
type Result = Extract<ToolPart, { type: "tool-result" }>;
...
export function prepareApprovalContext(
  ...
  // A handler may repeat a part (cloning the context, say), but every copy
  // must match the stored one, so the first copy is canonical.
  const requests = new Map<string, Request>();
  const calls = new Map<string, Call>();
  const responses = new Map<string, Response>();
  const results = new Map<string, Result>();
  const suppliedResponseIds = new Set<string>();
  for (const message of messages) {
    if (!Array.isArray(message.content)) continue;
    for (const part of message.content) {
      if (part.type === "tool-approval-response") {
        suppliedResponseIds.add(part.approvalId);
      }
      if (
        part.type === "tool-approval-request" &&
        approvalIds.has(part.approvalId)
      ) {
        remember(requests, part.approvalId, part);
      } else if (
        part.type === "tool-call" &&
        activeCallIds.has(part.toolCallId)
      ) {
        remember(calls, part.toolCallId, part);
      } else if (
        part.type === "tool-approval-response" &&
        approvalIds.has(part.approvalId)
      ) {
        remember(responses, part.approvalId, part);
      } else if (
        part.type === "tool-result" &&
        activeCallIds.has(part.toolCallId)
      ) {
        remember(results, part.toolCallId, part);
      }
    }
  }
  ...
  for (const message of storedMessages) {
    if (!Array.isArray(message.content)) continue;
    for (const part of message.content) {
      let supplied: Request | Call | Response | Result | undefined;
      if (
        part.type === "tool-approval-request" &&
        approvalIds.has(part.approvalId)
      ) {
        ...
        supplied = results.get(part.toolCallId);
        if (!supplied) {
          throw new Error(
      ...
      if (!supplied || !sameApprovalPart(supplied, part)) {
        throw new Error(
          `Continuation context must preserve the stored ${part.type} for ${partId(part)}`,
        );
      }
    }
  }
  if ([...suppliedResponseIds].some((id) => !storedResponseIds.has(id))) {
    throw new Error("Continuation context cannot add approval decisions");
  }
  if ([...results.keys()].some((id) => !storedResultIds.has(id))) {
    throw new Error(
  ...
  for (const message of messages) {
    ...
    const content = message.content.filter((part) => {
      ...
      if (part.type === "tool-approval-response") {
        if (deferredCallIds.has(storedRequests.get(part.approvalId) ?? "")) {
          return false;
        }
...
function remember<T extends ApprovalPart>(
  seen: Map<string, T>,
  id: string,
  part: T,
) {
  ...
  if (!first) {
    seen.set(id, part);
  } else if (!sameApprovalPart(first, part)) {
    throw new Error(
      `Continuation context must preserve the stored ${part.type} for ${partId(part)}`,
    );
  }
...
function partId(part: ApprovalPart): string {
...
// Part-level provider options are the handler's to add; everything else must
// match what was stored.
function sameApprovalPart(a: ApprovalPart, b: ApprovalPart): boolean {
  return approvalPartSignature(a) === approvalPartSignature(b);
}

function approvalPartSignature(part: ApprovalPart): string {
  switch (part.type) {
