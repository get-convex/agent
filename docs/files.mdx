...
### `Timeout while downloading <url>` from the provider

When a message carries an image or file as a URL, the component stores the URL
and hands it to the AI SDK. Providers that accept URLs for that media type
(OpenAI does for `image/*`) download it themselves; for other providers the AI
SDK downloads it inside your action before the request. If a provider error
reports `Timeout while downloading` and names the image URL, the provider could
not fetch that URL in time. The `AI_APICallError` request URL may be the
provider's API endpoint; that alone does not identify which fetch timed out.
Third-party hosts and CDNs can respond inconsistently, so the same image URL may
work on a later attempt.

To avoid relying on that host during generation, fetch the image in an action
and pass its bytes in the message. You can set the fetch timeout or add retries.
Bytes up to 64 KiB are sent inline, so no image URL fetch is needed; larger
images are saved to Convex file storage and the provider fetches a storage URL,
or the AI SDK fetches it for a provider that does not accept URLs. Either way
your host is out of the path:

```ts
export const ask = internalAction({
  args: { threadId: v.string(), imageUrl: v.string(), prompt: v.string() },
  handler: async (ctx, { threadId, imageUrl, prompt }) => {
    const url = new URL(imageUrl);
    if (
      url.origin !== "https://images.example.com" ||
      url.username ||
      url.password
    ) {
    ...
    if (!mediaType?.startsWith("image/")) {
      throw new Error(`Not an image: ${mediaType}`);
    }
...
Replace `images.example.com` with a host your application trusts. Do not fetch
arbitrary user-supplied URLs: a server that does so can be pointed at internal
hosts. Apart from localhost URLs, which it inlines for local development, the
component does not download URLs on your behalf.
