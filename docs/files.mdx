...
SDK downloads it inside your action before the request. A timeout with the
provider's API URL in the error (for example `AI_APICallError` from
`api.openai.com`) means the provider's fetch of your host timed out, not
anything in Convex. Third-party hosts and CDNs vary in how quickly they answer,
which is why the same URL can work one minute and fail the next.

The fix is to stop relying on that host. Fetch the image yourself in an action,
where you control the timeout and can retry, and pass the bytes in the message.
Bytes up to 64 KiB are sent inline, so nothing is fetched at all; larger images
are saved to Convex file storage and the provider fetches a storage URL, or the
AI SDK fetches it for a provider that does not accept URLs. Either way your host
is out of the path:
...
`AbortSignal.timeout` and a `signal` on `fetch` work in the Convex runtime even
though the runtime's API list does not mention them.

Only fetch URLs your application trusts. Apart from localhost URLs, which it
inlines for local development, the component does not download URLs on your
behalf, since a server that fetches whatever URL a user supplies can be pointed
at internal hosts.
