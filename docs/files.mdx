...
When a message carries an image or file as a URL, the component passes that URL
through and the model provider downloads it. A timeout with the provider's API
URL in the error (for example `AI_APICallError` from `api.openai.com`) means the
provider's fetch of your host timed out, not anything in Convex. Third-party
hosts and CDNs vary in how quickly they answer the provider, which is why the
same URL can work one minute and fail the next.
...
Bytes up to 64 KiB are sent to the provider inline, so nothing is fetched at
all; larger images are saved to Convex file storage and the provider fetches a
storage URL. Either way your host is out of the path:
...
Only fetch URLs your application trusts. The component does not download
arbitrary URLs on your behalf, since a server that fetches whatever URL a user
supplies can be pointed at internal hosts.
