...
### Agents that need `"use node"`

Queries and mutations must run in the Convex runtime, so `convex/playground.ts`
cannot import an agents module that needs Node APIs. Split the API in two:
queries in a Convex-runtime file, given the agents' metadata rather than the
instances, and actions next to the agents.
...
export const {
  ...
  agents: [{ name: "Weather Agent", tools: ["getWeather"] }],
});
```

```ts
// convex/playgroundNode.ts
"use node";
import { definePlaygroundActions } from "@convex-dev/agent";
import { components } from "./_generated/api";
import { weatherAgent } from "./example";

export const { generateText, fetchPromptContext } = definePlaygroundActions(
  components.agent,
  { agents: [weatherAgent] },
);
```

In the playground, set the API path to `playground` and the actions path to
`playgroundNode`. The `name` given to `definePlaygroundQueries` must match the
agent's `name` option, since the actions look the agent up by it.
`definePlaygroundAPI` composes `definePlaygroundQueries` and
`definePlaygroundActions` for the common case where one file can hold
everything.
