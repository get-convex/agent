...
vector search, you'll need an `embeddingModel`. A `name` is helpful to
...
const supportAgent = new Agent(components.agent, {
  ...
  tools: {
    ...
    myConvexTool: createTool({
      ...
      // Note: annotate the return type of the execute to avoid type cycles.
      execute: async (ctx, input): Promise<string> => {
        return "Hello, world!";
      },
    }),
    // Standard AI SDK tool
    myTool: tool({ description: "My tool", inputSchema: z.object({...}), execute: async () => {}}),
  },
