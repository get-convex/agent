...
A `generateText` or `streamText` call runs inside a single Convex action,
including every step of its tool loop. Actions in the Convex runtime can run for
30 minutes and Node actions (`"use node"`) for 10 minutes; see
[limits](https://docs.convex.dev/production/state/limits). An agent that runs
many tool calls for one user message can hit that ceiling.

For longer work, split generation across actions so each call finishes within
the limit. Completed steps are saved to the thread, and a later call can use
`promptMessageId` to start from the original prompt's history. That does not
resume a tool call that timed out or schedule the next action for you. The
Workflow component below can chain the calls durably; tools run in retried steps
