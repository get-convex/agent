...
Queries and mutations run in the Convex runtime, which has no Node.js APIs. If a
tool's `execute` imports a Node-only package, the whole module that defines the
agent fails to load there, and the error surfaces from whichever query or
mutation imports that module, typically the one listing thread messages.
...
module then stays loadable from the Convex runtime. To find which import pulls
in Node APIs, run:
