...
`httpAction`. The hook posts its `messages` array; take the last user message as
the prompt and let the agent load the rest of the history from the thread:
...
export const chat = httpAction(async (ctx, request) => {
  ...
  if (!last) return new Response("No messages", { status: 400 });
...
browser tab that made the request; the agent still persists every message, but
another tab or device sees nothing, and `useChat` never re-fetches on its own,
so on reload you seed its `messages` from a query over the thread.
`useUIMessages` reads the same persisted messages and deltas reactively from the
database, so every subscriber sees the stream. Use `useChat` when you need its
ecosystem, and `useUIMessages` when you need the conversation to be shared.
