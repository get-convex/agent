...
another tab or device sees nothing until it re-fetches. `useUIMessages` reads
the same persisted messages and deltas reactively from the database, so every
subscriber sees the stream. Use `useChat` when you need its ecosystem, and
