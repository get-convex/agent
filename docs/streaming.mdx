...
Convex client's auth token to an HTTP action. Forward your app's credentials
with the transport's `headers` or `credentials` option so the helper can check
ownership before generating a response.
