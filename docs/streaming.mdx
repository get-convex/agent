...
- Server: [streaming.ts](../example/convex/chat/streaming.ts)
- Client: [ChatStreaming.tsx](../example/ui/chat/ChatStreaming.tsx)
...
`toUIMessageStreamResponse()` returns the response shape `useChat` consumes, so
the AI SDK's React hook and the tooling built around it work against an
`httpAction`. The hook posts its `messages` array. Validate the last user
message, use its text as the prompt, and let the agent load the rest of the
...
const chatRequest = z.object({
  ...
  messages: z.array(
    z.object({ role: z.string(), parts: z.array(z.unknown()) }),
  ),
});
...
export const chat = httpAction(async (ctx, request) => {
  ...
  const prompt = last.parts
    ...
    .join("");
  if (!prompt) return new Response("No prompt text", { status: 400 });
...
Route it in `convex/http.ts` with CORS, as in
[http.ts](../example/convex/http.ts), then point the transport at it:
...
This example handles text-only user sends. Add handling for attachments or
regeneration if your chat UI supports them.

The example app's `authorizeThreadAccess` enforces thread ownership when its
third argument is `true`. `DefaultChatTransport` does not automatically send the
Convex client's auth token to an HTTP action. Pass it as an
`Authorization: Bearer <token>` header using the transport's `headers` option,
so the helper can check ownership before generating a response. See
[HTTP action auth](https://docs.convex.dev/auth/functions-auth#http-actions).

`VITE_CONVEX_SITE_URL` is not set by `npx convex dev`; add it to `.env.local`
yourself with your deployment's `.convex.site` URL (Dashboard, Settings, URL and
Deploy Key). It differs from the `.convex.cloud` value in `VITE_CONVEX_URL`.

The trade-off is where state lives. `useChat` holds the conversation in the
browser tab that made the request; the agent still persists every message, but a
new `useChat` instance does not automatically load or subscribe to that history.
On reload, seed its `messages` from a query over the thread. `useUIMessages`
reads the same persisted messages and deltas reactively from the database, so
every subscriber sees the stream. Use `useChat` when you need its ecosystem, and
`useUIMessages` when you need the conversation to be shared.
