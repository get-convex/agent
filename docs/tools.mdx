...
SDK's `hasToolCall` matches the call even if the tool fails, which can stop the
loop before the model has a chance to retry.
...
An artifact (a document, a code file, a canvas) can live in your application's
tables rather than in message content. One pattern is to store a version per
...
The mutation must authorize the write and allocate the next version. The tool
result stays small because it contains only the version, while the client reads
the artifact with `useQuery` and receives updates reactively. Keeping each
version as a separate row lets the app show earlier edits.
