...
After each step, the loop continues if the model finished with tool calls and no
`stopWhen` condition matched. `stopWhen` is the control over this. `maxSteps` is
a deprecated alias: `generateText` and `streamText` use it only when `stopWhen`
is not set, so passing both leaves you with just `stopWhen`.
...
Chef is a sandboxed code-generation loop with its own runtime and is not a
primitive to build this on.
