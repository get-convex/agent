...
`stopWhen` condition matched. `stopWhen` is the only control over this:
`maxSteps` is a deprecated alias that applies only when `stopWhen` is not set,
so passing both leaves you with just `stopWhen`.
