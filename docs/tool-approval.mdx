...
:::tip
You can approve a tool call and continue generation in the same server
function. For example, an action can call `approveToolCall` and then
immediately call `streamText` to resume — no separate scheduling step needed.
:::
