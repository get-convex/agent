...
thread contend on a single row. Keep the activity index in your app instead,
where the mutation that accepts a user message already runs:
...
Upsert the row by `by_threadId` in the same mutation that saves the prompt,
delete it by the same index when the thread is deleted, and list with the
activity index:
