...
### Listing threads by recent activity

Threads carry no `updatedAt`. Keeping one on the thread would mean patching the
thread document on every message, which makes concurrent generations in one
thread contend on a single row, and listing by it from the component would still
cost one read per thread. Keep the activity index in your app instead, where the
mutation that accepts a user message already runs:
...
}).index("by_userId_lastActivityAt", ["userId", "lastActivityAt"]),
```

Upsert the row in the same mutation that saves the prompt, delete it when the
thread is deleted, and list with the index:

```ts
const recent = await ctx.db
  .query("threadActivity")
  .withIndex("by_userId_lastActivityAt", (q) => q.eq("userId", userId))
  .order("desc")
  .paginate(paginationOpts);
```

One write per user turn, no component reads to list, and the ordering is
whatever your app defines as activity.
